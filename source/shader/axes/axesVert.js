...
void main() {
    ...
    if (iProjectionID == 100) {
        rayDir = iCamMat * normalize(vec3(pos.x, pos.y, focalLength));
    }
    ...
    vec3 lineSource = linePoint;
        if (lineType == 1)
            lineSource += iOriginOffset;

    vec3 closestAxisPoint = findClosestPoint(lineSource, lineDir, iCamPos, rayDir);
    float depth = length(closestAxisPoint - iCamPos) / cameraDepth;
